...
//                
//            }];
//            [[NSNotificationCenter defaultCenter]postNotificationName:@"token" object:nil userInfo:@{@"access_token":model.access_token}];
            [BmobUser signUpInBackgroundWithAuthorDictionary:@{@"access_token":model.access_token,@"uid":model.uid,@"expirationDate":model.expriresData} platform:BmobSNSPlatformSinaWeibo block:^(BmobUser *user, NSError *error) {
                if (user) {
                    ...
                    [[NSNotificationCenter defaultCenter]postNotificationName:USER_REFRESH_NOTICE object:model.access_token];
